...
def symbolic_test_reproducing_property_kernel_poly_Fock_monomial(n, m, p, q):
    ...
    r = diff1.is_zero()
    print('symbolic_test_repr_kernel_poly_Fock_monomial,')
    print('n = %d, m = %d, p = %s, q = %s,' % (n, m, str(p), str(q)))
...
def huge_parallel_symbolic_test_reproducing_property_kernel_poly_Fock_monomial(nmax, mmax, psum_max):
    print('huge_parallel_symbolic_test_reproducing_property_kernel_poly_Fock_monomial,')
    print('nmax = %d, mmax = %d, psum_max = %d\n' % (nmax, mmax, psum_max))
    t0 = time.time()
    samples = []
    for n in range(1, nmax + 1):
        for m in range(1, mmax + 1):
            ps = lists_with_bounded_sum(n, psum_max)
            qs = lists_with_bounded_sum(n, m - 1)
            samples += [(n, m, p, q) for p in ps for q in qs]
    print('number of samples: %d\n' % len(samples))
    results = list(symbolic_test_reproducing_property_kernel_poly_Fock_monomial(samples))
    huge_result = True
    for res in results:
        r = res[1]
        huge_result = huge_result and r
    t1 = time.time()
    print('total time = ' + str(t1 - t0))
    print('huge_result = ' + str(huge_result))
    return huge_result


print(huge_parallel_symbolic_test_reproducing_property_kernel_poly_Fock_monomial(2, 2, 5))
