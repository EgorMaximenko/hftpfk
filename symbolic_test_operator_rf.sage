# Here we test the formula for R_{\cF_m} K_{iy / sqrt{a}} from our paper
...
def example_vector_function_expr(n, m):
    # y is a parameter, xi is a variable
    al, u, v, y, xi = vars_aluvyxi(n)
    y2 = real_norm2(y)
    factor0 = 2 ** (- n / 2)
    factor1 = exp(y2 / 2)
...
def test_operator_rf_applied_to_kernel(n, m, verbose_level):
    ...
    result1 = example_vector_function_expr(n, m)
...
print(big_test_operator_rf_applied_to_kernel(3, 3, 1))
